...
const command : SlashCommand = {
    ...
    execute: async interaction => {
        ...
        switch (final){
            case 0:{await prisma.$disconnect();interaction.followUp({content:"please dont send malicius file",ephemeral:true});break}
            case 1:{await prisma.$disconnect();interaction.followUp({content:"file format invalid",ephemeral:true});break}
            case 2:{
        ...
        if (ch?.isTextBased()){ch.send(`${interaction.user.username}#${interaction.user.discriminator} transfer ${attachment.name} to server`)}
    },
    cooldown: 5
}
